...
| P4 | `prepare_video` returned the studio URL plus `clips_uploaded: false` and a browser next step; opening it as the same user loaded the project and the Make a video dialog. Using the returned read as the audio file and three local 1080×1920 clips, the page rendered a 21.7 MB MP4 on the device behind an explicit Save video button, which was not pressed. No completed video is claimed before that render. |
...
| Refresh | Pending at the time of this commit: a scheduled check after the one-hour access-token window (connection approved 20:04:43 UTC) will be recorded in a follow-up commit. |
